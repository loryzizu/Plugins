...
public class TableToRdfConfigurator {
     ...
     * @param header
     * @param data
     *            Contains first data row, or ColumnType if type is already known.
     * @param numberOfLeadingEmpty
     *            Number of leading empty columns, this is useful for xsl-like.
    ...
    public static void configure(TableToRdf tableToRdf, List<String> header,
            List<Object> data, int numberOfLeadingEmpty) throws ConversionFailed, DPUException {
        // initial checks
        if (data == null) {
            throw new ConversionFailed("First data row is null!");
        }
        if (header != null && header.size() != data.size()) {
            throw new ConversionFailed("Diff number of cells in header ("
